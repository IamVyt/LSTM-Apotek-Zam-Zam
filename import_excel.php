...
<head>
    ...
    <style>
        ...
        .log-ok { color: #065f46; background: #d1fae5; padding: 14px 18px; border-radius: 8px; margin-bottom: 12px; border-left: 4px solid #10b981; font-weight: 600; font-size: 0.95rem; }
        .log-err { color: #991b1b; background: #fee2e2; padding: 14px 18px; border-radius: 8px; margin-bottom: 12px; border-left: 4px solid #ef4444; font-weight: 600; font-size: 0.95rem; }
        .log-info { color: #1e40af; background: #dbeafe; padding: 14px 18px; border-radius: 8px; margin-bottom: 12px; border-left: 4px solid #3b82f6; font-weight: 600; font-size: 0.95rem; }
...
if ($useCsv) {
    ...
    foreach ($xlsx->sheetNames() as $idx => $name) {
        if (stripos($name, 'Data Mingguan') !== false) { $sheetIndex = $idx; break; }
    }
    if ($sheetIndex === -1) die("<div class='log-err'>Sheet 'Data Mingguan' tidak ditemukan di Excel.</div>");
...
foreach ($rows as $index => $row) {
    ...
    if (empty($row[1]) || empty($row[3])) continue;   // Skip baris kosong
    ...
    foreach ($obatMap as $keyword => $id) {
        if (stripos($namaObatStr, $keyword) !== false) { $obatId = $id; break; }
    }
